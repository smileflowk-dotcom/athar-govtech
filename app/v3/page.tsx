  ...
  return <main className={styles.shell}>
    ...
    {view === "dossiers" && <>
      <section className={styles.heroPanel}>
        ...
        <label className={styles.uploadCard}>
          ...
          <small>Les autres formats prévus par la cible institutionnelle seront activés avec la chaîne documentaire adaptée.</small>
      ...
      <section className={styles.workspaceGrid}>
        ...
        <section className={styles.panel}>
          <div className={styles.dossierTop}><div><span className={styles.eyebrow}>DOSSIER SÉLECTIONNÉ</span><h2>{dossier.title}</h2><p>{dossier.buyer ?? "Acheteur non renseigné"}</p></div><button className={styles.primary} onClick={() => setView("controler")}>Contrôler le dossier</button></div>
          ...
          <div className={styles.readyLine}><FileSearch2 size={18}/><div><strong>Dossier prêt pour contrôle</strong><span>Les points sont présentés avec règle, fait observé et preuve source.</span></div></div>
        </section>
      </section>
    </>}
    ...
    {view === "prouver" && alert && <>
      ...
      <div className={styles.proof}>
        <section className={styles.panel}><span className={styles.eyebrow}>POURQUOI ATHAR LE SIGNALE</span><h2>{alert.rule}</h2><div className={styles.compare}><div><span>ATTENDU</span><p>{alert.expected}</p></div><div><span>OBSERVÉ</span><p>{alert.observed}</p></div></div><div className={styles.sourceHeading}><FileSearch2 size={17}/><strong>Preuve source</strong></div><div className={styles.sourceStack}>{evidenceItems.map((evidence) => <div className={styles.quote} key={evidence.id}><strong>{evidence.source} · {evidence.location}</strong><p>{evidence.excerpt}</p></div>)}</div></section>
        <aside className={`${styles.panel} ${styles.decision}`}><span className={styles.eyebrow}>VALIDATION HUMAINE</span><h2>{statusLabel[alert.status]}</h2><p className={styles.hint}>ATHAR signale. Le contrôleur décide.</p><textarea rows={7} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Justifier la décision à partir de la preuve…"/><div className={styles.actions}><button className={styles.primary} disabled={note.trim().length < 8} onClick={() => decide("confirmed")}><Check size={16}/> Confirmer</button><button className={styles.button} disabled={note.trim().length < 8} onClick={() => decide("requested")}>Demander une pièce</button><button className={styles.button} disabled={note.trim().length < 8} onClick={() => decide("dismissed")}><CircleX size={16}/> Écarter</button></div><small className={styles.auditNote}>La décision humaine reste distincte du signal produit par ATHAR.</small></aside>
      </div>
    </>}

    {view === "livrer" && <>
      <div className={styles.bar}><div><span className={styles.eyebrow}>04 · LIVRER</span><h1>Livrables du contrôle</h1><p className={styles.hint}>Seuls les constats confirmés alimentent les livrables finaux.</p></div></div>
      <section className={styles.panel}><div className={styles.deliverables}><article className={styles.deliverable}><FileCheck2 size={24}/><h3>Synthèse des constats</h3><p>{confirmed.length} constat(s) confirmé(s), chacun relié à sa règle, sa preuve et sa validation.</p><button className={styles.primary} disabled={!confirmed.length} onClick={() => setPreview(true)}>Prévisualiser</button></article><article className={styles.deliverable}><ShieldCheck size={24}/><h3>Dossier de preuves</h3><p>Sources, extraits utiles et trace de la validation humaine regroupés pour la revue.</p></article></div></section>
    </>}
