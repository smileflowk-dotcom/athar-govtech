...
function extractionMetadata(pages: ReturnType<typeof normalizeExtractedPages>) {
  return {
    scripts: summarizeDocumentScripts(pages),
    tablePages: pages.filter((page) => page.hasTableContent).map((page) => page.page),
  };
}
...
export async function POST(request: Request) {
  try {
    ...
    const nativePages = normalizeExtractedPages(await extractNativePdf(bytes));
    ...
    if (!shouldUseEnhancedExtraction(nativeQuality)) {
      return NextResponse.json({
        ...
        documentProfile: extractionMetadata(nativePages),
    ...
    if (doclingEnabled()) {
      try {
        ...
        const enhancedPages = normalizeExtractedPages(await extractWithDocling(enhancedFile));
        ...
        return NextResponse.json({
          ...
          documentProfile: extractionMetadata(enhancedPages),
